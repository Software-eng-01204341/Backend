...
// DeletePostCascade performs a hard delete of a post and the minimum related records
// that can affect frontend: likes on the post, likes on comments under the post,
// and comments of the post. It intentionally leaves hashtags/post_categories/
// post_role_visibility as-is per current policy since inactive/absent posts are
// already filtered out on reads.
func DeletePostCascade(db *mongo.Database, postID bson.ObjectID, ctx context.Context) (bool, error) {
    ...
    // Ensure post exists before cascading deletes
    var chk struct{ ID bson.ObjectID `bson:"_id"` }
    if err := postsCol.FindOne(ctx, bson.M{"_id": postID}, options.FindOne().SetProjection(bson.M{"_id": 1})).Decode(&chk); err != nil {
    ...
    // 2) Collect comment IDs under this post, then delete likes on those comments
    ...
    // 3) Delete comments under this post
